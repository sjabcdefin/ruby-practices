...
def configure_max_nlink_size_block(files)
  ...
  files.each do |file|
    ...
    size_max_array << file_attribute.size.to_s.size
    nlink_max_array << file_attribute.nlink.to_s.size
    number_of_block += file_attribute.blocks
  end
  [size_max_array, nlink_max_array, number_of_block]
end

def display_attributes(files)
  size_max_array, nlink_max_array, number_of_block = configure_max_nlink_size_block(files)
  puts "合計 #{number_of_block / 2}"
  files.each do |file|
    file_attribute = File::Stat.new(file)
    ...
    display_items = [
      ...
      format("%#{nlink_max_array.max}d", file_attribute.nlink),
      FILE_USER[file_attribute.uid],
      FILE_GROUP[file_attribute.gid],
      format("%#{size_max_array.max}d ", file_attribute.size).ljust(size_max_array.max),
